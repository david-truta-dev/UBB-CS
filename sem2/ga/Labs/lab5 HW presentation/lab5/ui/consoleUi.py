...
class ConsoleUi:
    ...
    def menu():
        print("\tAvailable commands:\n"
              ...
              "'HCLC' - constructs a minimal spanning tree and prints it.\n"
    ...
    def uiGetBound(self):
        vertex = self.uiReadVertex()
        start = time.time()
        print("Nr of inbound:", self._service.graph.getDegree(vertex))
        lst = list(self._service.graph.boundEdge(vertex))
        print(lst)
        end = time.time()
        print("Command took", end - start, "to execute.\n")
    ...
    def uiHCLC(self):
        start = time.time()
        graph = self._service.HamiltonCycleLowCost()
        if graph is None:
    ...
    def run(self):
        commandDict = {'read': self.uiReadFromTextFile, 'write': self.uiWriteGraphToFile, 'print': self.uiPrintGraph,
                       ...
                       'get inbound': self.uiGetBound, 'get nr of vert': self.uiGetNrOfVert,
                       'get nr of edge': self.uiGetNrOfEdges, 'kruskal': self.uiKruskal, 'hclc': self.uiHCLC,
