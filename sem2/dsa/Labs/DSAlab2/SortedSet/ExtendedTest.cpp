...
void testRemoveIterator(Relation r) {
	cout << "Test RemoveIter" << endl;
	SortedSet s(r);
	s.add(5);
	s.add(1);
	s.add(10);
	...
	while (it.valid()) {
		e = it.getCurrent();
		if (e == 7) {
			assert(7 == it.removeCurrent());
			assert(10 == it.getCurrent());
		}
		if (e == 10) {
			assert(10 == it.removeCurrent());
			try {
				e = it.removeCurrent();
				assert(false);
			}
